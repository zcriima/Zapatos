...
function renderFilters(){
  ...
  filters.forEach(f => {
    ...
    btn.className = active === f
      ? "filter-btn px-4 py-2 rounded-full border text-sm bg-black text-white border-black"
      : "filter-btn px-4 py-2 rounded-full border text-sm bg-white/75 border-white/70";
    btn.onclick = () => {
      active = f;
      renderFilters();
      renderProducts();
