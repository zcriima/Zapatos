...
function renderFilters(){
  ...
  filters.forEach(f => {
    ...
    const activeClass = active === f
      ? "bg-black text-white border-black"
      : "bg-white text-gray-800 border-gray-300 hover:bg-gray-100";

    btn.className = `px-4 py-2 rounded-full border text-sm transition ${activeClass}`;
...
function renderProducts(){
  ...
  list.forEach(p => {
    ...
    card.innerHTML = `
      <button class="open-image block w-full bg-gray-100" data-image="${p.image}" data-name="${p.name}">
      ...
      <div class="p-4">
        ...
        <p class="text-sm text-gray-500 mt-1">Tallas: ${p.sizes}</p>
  ...
  document.querySelectorAll(".open-image").forEach(button => {
    button.addEventListener("click", () => {
      openModal(button.dataset.image, button.dataset.name);
    });
  });
}
